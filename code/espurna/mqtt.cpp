...
void mqttSend4Balance(const char * topic, const char * message) {

    // Enqueue new message
    mqttEnqueue(topic, message);

    // Reset flush timer
    _mqtt_flush_ticker.once_ms(MQTT_USE_JSON_DELAY, mqttFlush4Balance);
}
...
void mqttFlush() {
    ...
    root[MQTT_TOPIC_TURN] = String(g_number_n);
    if (ntpSynced()) 
    {
        root[MQTT_TOPIC_TIME] = ntpDateTime();
    } else 
    {
        root[MQTT_TOPIC_TIME] = INVALID_TIME;
    }
    #endif
...
void mqttFlush4Balance() {
    ...
    root[MQTT_TOPIC_TURN] = String(g_mqtt_packet_data.number_n);
    #if 0
    if (ntpSynced()) 
    {
        root[MQTT_TOPIC_TIME] = ntpDateTime();
    ...
    #endif
    root[MQTT_TOPIC_POND] = g_mqtt_packet_data.pondID_str;
    root[MQTT_TOPIC_TYPE] = g_mqtt_packet_data.type_str;
