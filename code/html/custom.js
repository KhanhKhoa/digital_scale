...
function doSendPondID() {
    var getpondID = $("input[name='pondid']").val();
    var getType = $('select[name="sltype"] option:selected').text();
    sendAction("pondInfo", {getpondID: getpondID, typeS: getType});
    return false;
}
...
function processData(data) {
    ...
    Object.keys(data).forEach(function(key) {
        ...
        if ("backPondID" === key) {
            var backPondID = data.backPondID;
            $("input[name='pondid']").val(backPondID);
            return;
        }
        ...
        if ("backType" === key) {
            ...
            if(backType === "Tôm thẻ")
            {
                $('select[name="sltype"] option[value="type_1"]').prop('selected', true);
            } else if(backType === "Tôm sú")
            {
                $('select[name="sltype"] option[value="type_2"]').prop('selected', true);
            } else
            {
                $('select[name="sltype"] option[value="type_3"]').prop('selected', true);
            }
            return;
        }
...
$(function() {
    ...
    $(".button-send-pondid").on("click", doSendPondID);
