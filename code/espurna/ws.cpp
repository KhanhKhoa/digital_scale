...
void _wsParse(AsyncWebSocketClient *client, uint8_t * payload, size_t length) {
    ...
    if (action) {
        ...
        if (strcmp(action, POND_INFO_ROOT) == 0) {
            const char* m_pond = root[DATA_ROOT][POND_ID_ROOT];
            const char* m_type = root[DATA_ROOT][TYPE_ROOT];
            String pond_str(m_pond);
            String type_str(m_type);
            if((pond_str.length() <= MAX_LENGHT_POND_TYPE)&&(type_str.length() <= MAX_LENGHT_POND_TYPE))
            {
                DEBUG_MSG_P(PSTR("[WEBSOCKET] action: %s; pondid: %s; type: %s\n"), action, m_pond, m_type);
                g_mqtt_packet_data.pondID_str = pond_str;
                g_mqtt_packet_data.type_str   = type_str;

                EEPROMr.write(PONDID_LENGHT_ADDRESS, pond_str.length());
                EEPROMr.write(TYPE_LENGHT_ADDRESS, type_str.length());

                for(uint8_t i = WS_DATA_ADDRESS; i< type_str.length() + pond_str.length() + WS_DATA_ADDRESS; i++)
                {
                    if(i < pond_str.length() + WS_DATA_ADDRESS)
                        EEPROMr.write(i, pond_str[i-WS_DATA_ADDRESS]);
                    else
                        EEPROMr.write(i, type_str[i-pond_str.length()-WS_DATA_ADDRESS]);
                }

                EEPROMr.commit();
                wsSend_P(client_id, PSTR("{\"message\": 11}"));
            }
            else
            {
                DEBUG_MSG_P(PSTR("[WEBSOCKET] Invalid pondID or typeID\n"));
                wsSend_P(client_id, PSTR("{\"message\": 12}"));
                return;
            }
        }
